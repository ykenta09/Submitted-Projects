...
///	インプットレイアウト
///	</summary>
const std::vector<D3D11_INPUT_ELEMENT_DESC> CircularFade::INPUT_LAYOUT =
{
...
void CircularFade::LoadTexture(const wchar_t* path)
{
...
void CircularFade::Create(DX::DeviceResources* pDR)
{
   m_pDR = pDR;
...
///	Shader作成部分だけ分離した関数
///	</summary>
void CircularFade::CreateShader()
{
   ID3D11Device1* device = m_pDR->GetD3DDevice();

   //	コンパイルされたシェーダファイルを読み込み
   BinaryFile VSData = BinaryFile::LoadFile(L"Resources/Shaders/CircularFade/CircularFadeVS.cso");
   BinaryFile GSData = BinaryFile::LoadFile(L"Resources/Shaders/CircularFade/CircularFadeGS.cso");
   BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/CircularFade/CircularFadePS.cso");
...
void CircularFade::Render()
{
   m_time += 0.025f;

   if (m_isFadeIn)	 //フェードインの場合
   {
	  if (m_time >= 1.0f)
	  {
		 m_isFadeEnd = true;
