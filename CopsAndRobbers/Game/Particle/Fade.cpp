...
void Fade::Render()
{
   ...
   cbuff.time = DirectX::SimpleMath::Vector4(m_time, (m_isFadeIn ? 1.0f : 0.0f), (m_isFadeOut ? 1.0f : 0.0f), 0);
   ...
   if (m_isFadeIn && m_time >= 1.0f)	 //フェードインの場合
   {
	  m_isFadeIn = false;
	  m_isFadeInEnd = true;
	  m_isFadeStarted = false;
   }
   if (m_isFadeOut && m_time >= 1.0f)	 //フェードアウトの場合
   {
	  m_isFadeOut = false;
	  m_isFadeOutEnd = true;
   }
