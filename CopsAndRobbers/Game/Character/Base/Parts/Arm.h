...
//腕クラスを定義する
class Arm : public ILeaf
{
public:
   static constexpr float LEFT_ARM_INIT_Z_ROT_DEG = -45.0f;   // 左腕の初期Z回転（度）
   static constexpr float RIGHT_ARM_INIT_Z_ROT_DEG = 45.0f;   // 右腕の初期Z回転（度）
   static constexpr float ARM_SWING_INIT_DEG = -3.0f;   // 腕振りの初期角度（度）
   static constexpr float ARM_SWING_LIMIT_DEG = 30.0f;   // 腕振りの最大角度（度）
   static constexpr float ARM_SWING_DECAY = 0.9f;    // 腕振りの減衰率
   static constexpr float ARM_ROT_OFFSET_Y = 0.0f;    // 初期Y回転（度）
public:
	//親を取得する
